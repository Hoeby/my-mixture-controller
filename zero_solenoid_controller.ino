//this code is written for the Arduino Zero by david taylor and released under the 
...
//pin d12 is the rc channel input
...
// a flight. I estimate anything between this temp and 35 °F higher will cause the engine
// to run rich. Set fallback temp to the lowest temperature you're likely to fly in.
const int fallback_temp = 0;
const int fallback_pressure = 102000;
...
//check for bad data from bmp280 and set error flag
  if (bmp.readTemperature() >= 85) bmp_error = 1;
  if (bmp.readPressure() >= 110000) bmp_error = 1;                  
...
// get temperature and pressure from library
  ...
  if (temp >= 85) temp = fallback_temp;
  if (pressure >= 110000) pressure = fallback_pressure;  
  
//adjust for temp and pressure  
    adjusted_pulse = (usable_pulse * ((reference_temp/(temp + 273))*(pressure/reference_pressure))) * 6.145;
...
//print correction
  ...
  float corr =  1000 * (reference_temp/(temp+273)*(pressure/reference_pressure)); 
  if(corr>=1000){
